...
public class GearTest extends Utility {
    ...
    public void userShouldAddProductSuccessFullyToShoppingCart() {
        ...
        //Verify the text ‘Overnight Duffle’
        Assert.assertEquals("Overnight Duffle", getTextFromTheElement(By.xpath("//span[@class='base']")));
        ...
        Assert.assertEquals("You added Overnight Duffle to your shopping cart.", getTextFromTheElement(By.xpath("//div[@data-bind='html: $parent.prepareMessageForHtml(message.text)']")));

        //Click on ‘shopping cart’ Link into message
        clickOnElement(By.linkText("shopping cart"));

        //Verify the product name ‘Overnight Duffle’
        Assert.assertEquals("Overnight Duffle", getTextFromTheElement(By.partialLinkText("Overnight Duf")));

        //Verify the Qty is ‘3’
        Assert.assertEquals("3", driver.findElement(By.xpath("//input[@class='input-text qty']")).getAttribute("value"));

        //Verify the product price ‘$135.00’
        Assert.assertEquals("$135.00", getTextFromTheElement(By.xpath("//span[@class='cart-price']//span[@class='price'][normalize-space()='$135.00']")));
        ...
        //Verify the product price ‘$225.00
        Assert.assertEquals("$225.00", getTextFromTheElement(By.xpath("//span[@class='cart-price']//span[@class='price'][normalize-space()='$225.00']")));
    }
